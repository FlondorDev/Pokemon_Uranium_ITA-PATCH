  ...
  def pbCompileText(file, out)
    outfile=File.open(out,"wb")
    begin
      intldat=pbGetText(file)
  ...
  class Messages
    ...
    def extract(outfile)
      origMessages=Messages.new(@filename)
      return false if !origMessages
    ...
    def dumpMessageFile(filename=nil)
  ...
  def _MAPINTL(mapid,*arg)
    arg[0].gsub!(/\n/,"\\n") #L replace actual newline with newline character.
    #echoln("Translate: #{arg[0]}")
    string=MessageTypes.getFromMapHash(mapid,arg[0]) #Translate
    string=string.clone
    for i in 1...arg.length
      string.gsub!(/\{#{i}\}/,"#{arg[i]}")
    end
    string.gsub!(/\\n/,"\n") #L Replace newline character with actual new line.
    return string
  end
